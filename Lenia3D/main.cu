...
int main(int argc, char ** argv) {
	...
	if (argv[10][0] == 'c') {
		LeniaEnvironmentCpu::KernelType kernelType;
		switch (argv[7][0]) {
		case 'u':
			kernelType = LeniaEnvironmentCpu::KernelType::kUniform;
			break;
		case 'm':
			kernelType = LeniaEnvironmentCpu::KernelType::kMove;
			break;
		case 'e':
			kernelType = LeniaEnvironmentCpu::KernelType::kExponential;
			break;
		case 'p':
			kernelType = LeniaEnvironmentCpu::KernelType::kPolynomial;
			break;
		case 'r':
			kernelType = LeniaEnvironmentCpu::KernelType::kRectangular;
			break;
		case 'g':
			kernelType = LeniaEnvironmentCpu::KernelType::kGol;
			break;
		default:
			std::cerr << "Unknown kernel type: " << argv[7][0] << std::endl;
			throw std::runtime_error("Unknown kernel type");
		...
		LeniaEnvironmentCpu::GrowthType growthType;
		switch (argv[8][0]) {
		case 'd':
			growthType = LeniaEnvironmentCpu::GrowthType::kDouble;
			break;
		case 'e':
			growthType = LeniaEnvironmentCpu::GrowthType::kExponential;
			break;
		case 'p':
			growthType = LeniaEnvironmentCpu::GrowthType::kPolynomial;
			break;
		case 'r':
			growthType = LeniaEnvironmentCpu::GrowthType::kRectangular;
			break;
		default:
			std::cerr << "Unknown growth type" << std::endl;
			throw std::runtime_error("Unknown growth type");
		...
		LeniaEnvironmentCpu env(argv[9], fieldSize, kernelRadius, stepsPerTimeUnit, mu, sigma,
			kernelType, growthType);
