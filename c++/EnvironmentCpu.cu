...
EnvironmentCpu::EnvironmentCpu(std::string file, int fieldSize, int kernelRadius, int stepsPerTimeUnit, float mu, float sigma,
...
void EnvironmentCpu::NormalizeKernel() {
	float sum = 0.0;
...
void EnvironmentCpu::SetUniformKernel() {
	for (int idx = 0; idx < volumeKernel; idx++)
		kernel[idx] = 1.0;
	NormalizeKernel();
}

void EnvironmentCpu::SetMoveKernel() {
	kernel[0] = 1.0;
	NormalizeKernel();
}

void EnvironmentCpu::SetExponentialKernel() {
	int mid = padSize;
...
void EnvironmentCpu::SetPolynomialKernel() {
	int mid = padSize;
	int padSizeSqr = padSize * padSize;
	for (int lay = 0; lay < kernelSize; lay++) {
		float laySqr = (lay - mid) * (lay - mid);
...
void EnvironmentCpu::SetRectangularKernel() {
	int mid = padSize;
	int padSizeSqr = padSize * padSize;
	for (int lay = 0; lay < kernelSize; lay++) {
		float laySqr = (lay - mid) * (lay - mid);
...
void EnvironmentCpu::SetGolKernel() {
	int mid = padSize;
	int padSizeSqr = padSize * padSize;
	for (int lay = 0; lay < kernelSize; lay++) {
		float laySqr = (lay - mid) * (lay - mid);
...
void EnvironmentCpu::ApplyDoubleGrowth() {
	int fieldSizeSqr = fieldSize * fieldSize;
	for (int lay = 0; lay < fieldSize; lay++) {
		for (int col = 0; col < fieldSize; col++) {
			for (int row = 0; row < fieldSize; row++) {
...
void EnvironmentCpu::ApplyExponentialGrowth() {
	int fieldSizeSqr = fieldSize * fieldSize;
	for (int lay = 0; lay < fieldSize; lay++) {
		for (int col = 0; col < fieldSize; col++) {
			for (int row = 0; row < fieldSize; row++) {
...
void EnvironmentCpu::ApplyPolynomialGrowth() {
	int fieldSizeSqr = fieldSize * fieldSize;
	for (int lay = 0; lay < fieldSize; lay++) {
		for (int col = 0; col < fieldSize; col++) {
			for (int row = 0; row < fieldSize; row++) {
...
void EnvironmentCpu::ApplyRectangularGrowth() {
	int fieldSizeSqr = fieldSize * fieldSize;
	for (int lay = 0; lay < fieldSize; lay++) {
		for (int col = 0; col < fieldSize; col++) {
			for (int row = 0; row < fieldSize; row++) {
...
void EnvironmentCpu::ApplyGrowth() {
	switch (growthType) {
...
void EnvironmentCpu::Step() {
	Convolute();
	ApplyGrowth();
	SumAndCut();
}

float* EnvironmentCpu::GetResult() {
	return imgCpu;
}

float* EnvironmentCpu::StepTimeUnit() {
...
void EnvironmentCpu::PutStepTimeUnit(std::string file) {
	float* result = StepTimeUnit();
	std::ofstream output(file, std::ios::binary);
	output.write((char *)result, volume * sizeof(*result));
}

void EnvironmentCpu::PutResult(std::string file) {
